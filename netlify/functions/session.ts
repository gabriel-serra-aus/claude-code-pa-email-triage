...
/** A proxy that compresses the response may hand the etag back weakened. */
...
async function get(): Promise<Response> {
  ...
  return respond(200, JSON.stringify(stored.doc, null, 2), { "Content-Type": "application/json", ETag: stored.etag });
}
...
async function put(req: Request): Promise<Response> {
  ...
  return json(200, { ok: true }, { ETag: written.etag });
}
