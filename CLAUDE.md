...
- **Gmail labels are email-only**: `labels` (now) and `decision.labels` (wanted), clickable chips on the head row, `labelOptions()` = `gmailLabels ∪ already-set`. They mirror nothing on a task — their only task-side effect is the title prefix.
