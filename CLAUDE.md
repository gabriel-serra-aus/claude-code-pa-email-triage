...
- Non-tracked emails: Not Important → `archive` (locked); FYI → `archive` (default) | `flag`; Important → `create-task` (default) | `flag` | `archive`. Every email ends up flagged (kept in inbox) or archived — no leave-alone, no create-email. `flag` / `create-task` / `update-task` flag it; `archive` / `complete-task` / `cancel-task` archive it.
...
The save skill runs only once neither group is `pending-review`, then processes exactly the groups whose status is `reviewed`. The top-level `status` is a derived summary (`aggregateStatus()`) that nothing keys off. `ensureGroups()` migrates legacy files (`status` / `reviewedGroups`, including an orphan `reviewedGroups.<tab>` stamp next to a `pending-review` `groups.<tab>`) on load and drops `reviewedGroups`.
